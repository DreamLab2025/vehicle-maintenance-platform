...
Run `task app:build` (or `dotnet build Verendar.sln`) from the solution root and analyze the output. For each error:
...
- New required constructor parameter not passed in service/repository
- EF Core configuration not matching entity changes
- MassTransit consumer not registered after adding new event contract

After fixing, run `task app:build` again to confirm clean build.
