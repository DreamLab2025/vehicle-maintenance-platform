...
Accepts a GitHub Actions run URL (e.g. `https://github.com/owner/repo/actions/runs/123456`) or a description of the CI failure.
...
| Type | Signal |
|---|---|
| Build failure | Compilation errors in `dotnet build` step |
...
**Why CI fails but local passes** (if applicable): environment difference (missing seed data, different .NET version, no Docker infrastructure, env vars not set).
...
Do not implement the fix — only plan it. To proceed to implementation, use `/fix:ci` with the same URL/description.
