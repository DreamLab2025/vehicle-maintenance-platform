...
Identify the working mode before doing anything else — this shapes how each subsequent step runs:
...
- **Log-driven** — error surfaced via logs or Seq
...
- Explain _why_ it isn't obvious (hidden coupling, async timing, EF tracking, RBAC logic, etc.)
...
- For bugs: fix in the correct Clean Architecture layer (`Domain → Application → Infrastructure → Api`)
- For CI config: follow existing workflow patterns — never use `--no-verify`
- For log-driven: minimal fix — don't refactor surrounding code unless it is the root cause
- For test fixes: fix the implementation if it's broken; update the test only if the behavior intentionally changed
...
```bash
task test:filter -- <AffectedClassName>   # targeted
task test                                  # full suite if fix touches shared code
```
