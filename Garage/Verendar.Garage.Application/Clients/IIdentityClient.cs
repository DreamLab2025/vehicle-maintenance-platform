...
{
    Task<(Guid? UserId, string? ActualPassword)> CreateMechanicUserAsync(CreateMemberUserRequest request, CancellationToken ct = default);
    Task<(Guid? UserId, string? ActualPassword)> CreateManagerUserAsync(CreateMemberUserRequest request, CancellationToken ct = default);
